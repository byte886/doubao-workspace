...
> 最后更新：2026-09-15
> 本文档记录两台机器的凭证位置、管理方式与使用规范。敏感值不在这里明文存储，只记录位置与获取方式。
...
- **注意**：在脚本中使用 `echo '***REMOVED***' | sudo -S <command>` 可免交互执行；但仅限可信脚本，不要在日志或输出中暴露
...
5. **双机同步**：SSH 密钥两台机器保持一致；git 提交统一在 wenjiechen 机执行
